        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            items(viewModel.services) { service ->
                Card(
                    ...
                    colors = CardDefaults.cardColors(containerColor = Color(0xFFFF5722)) // Darker orange
                ) {
                    Column(modifier = Modifier.padding(12.dp)) {
                        Text(service.name, fontWeight = FontWeight.Bold, fontSize = 16.sp, color = Color.White)
                        Text("Ksh ${service.price}", color = Color(0xFFFFFFFF), fontSize = 14.sp)
...
@Preview(showBackground = true, backgroundColor = 0xFFFF9800)
@Composable
